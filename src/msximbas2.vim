...
let s:version='0.2.3+201611011725'
...
" This program converts an MSX-imBASIC source to an MSX-BASIC file, optionally
" in a DSK disk image.
...
" In order to create a DSK file, this program needs `wrdsk, an utility writted
" by Arnold Metselaar in 1996..1998.  The original binaries for MS-DOS (which
" could be used with DOSemu) can be found in several MSX websites, but the
" sources (included with MSX-imBASIC) are in <http://fms.komkon.org/fMSX/>.

" ----------------------------------------------

function! MSXimBASICClean()
...
function! MSXimBASICControlStructures()

  call MSXimBASICDoLoop()
  call MSXimBASICExitFor()
  call MSXimBASICIfEndif()
  call MSXimBASICProcedures()

endfunction

function! MSXimBASICDoLoop()

  " Convert all 'DO LOOP' structures.

  " The MSXimBASIC BASIC 'DO LOOP' structures are copied from Andy Wright's Beta
  ...
  call MSXimBASICExitDo()

  call MSXimBASICSaveStep('do_loop')

endfunction

function! MSXimBASICDo()
  ...
  " 's:doStatement' in order to create it later ('MSXimBASICLoop()' does it),
...
function! MSXimBASICExitFor()
  ...
  call MSXimBASICSaveStep('exit_for')

endfunction

function! MSXimBASICIfEndif()

  " Convert all 'IF ENDIF' structures.

  " XXX TODO finish -- adapt to MSX-BASIC, which provides `ELSE`

  " The MSX-imBASIC's 'IF ENDIF' structures are inspired by Andy Wright's
  " Beta BASIC, SAM BASIC and MasterBASIC, but they are not identical.

  " Syntax:

  " Short 'IF' structures are the same than MSX-BASIC's.
...
"        echo '  <'.getline('.').'>'
        if l:unclosedConditionals==1 " current IF structure?
          if l:elseLineNumber " there was a previous ELSE?
            ...
            let l:newIf='if '.MSXimBASICNot(l:condition).' then goto '.l:elseIfLabel
...
"        echo '  <'.getline('.').'>'
        if l:unclosedConditionals==1 " current IF structure?
          if l:elseLineNumber " there was a previous ELSE?
            ...
            let l:newIf='if '.MSXimBASICNot(l:condition).' then goto '.l:elseLabel
            call setline(l:conditionLineNumber,l:newIf)
...
"        echo '  <'.getline('.').'>'
        ...
        if l:unclosedConditionals==0 " current IF structure?
          ...
          if len(l:condition) " is there an unresolved condition?
            let l:newIf='if '.MSXimBASICNot(l:condition).' then goto '.l:endifLabel
            call setline(l:conditionLineNumber,l:newIf)
          endif
          break
        endif
  ...
  call MSXimBASICSaveStep('if_endif')

endfunction

function! MSXimBASICNot(expression)
  ...
  if match(l:expression,'=0$')==(len(l:expression)-2)
    ...
    let l:expression=MSXimBASICWithoutParens(l:expression)
  else
...
function! MSXimBASICWithoutParens(expression)
...
function! MSXimBASICProcedures()
  ...
  call MSXimBASICSaveStep('procedures')
...
function! MSXimBASICDoVim(directive)
  ...
  call MSXimBASICSaveStep(strpart(a:directive,1).'_directives')

endfunction

function! MSXimBASICVim()

  " Search for all '#previm' and '#vim' directives and execute
  " their Vim commands.

  call MSXimBASICDoVim('#previm')
  call MSXimBASICDoVim('#vim')
...
function! MSXimBASICInclude()
  ...
  call MSXimBASICSaveStep('included_files')

endfunction

function! MSXimBASICConditionalConversion()
...
"        echo 'XXX #endif found'
  ...
  call MSXimBASICSaveStep('conditional_conversion')

endfunction

function! MSXimBASICIfdef()
...
"    echo 'XXX l:tagMustBeDefined='.l:tagMustBeDefined
    let l:tagIsDefined=MSXimBASICDefined(l:tag)
...
function! MSXimBASICConfig()
  ...
  call MSXimBASICRenumLine()
  " #procedureCall <keyword>
  call MSXimBASICProcedureCall()
  " #run <label or line number>
  call MSXimBASICRun()
  " #define <tag>
  call MSXimBASICDefine()

  call MSXimBASICSaveStep('config_values')

endfunction

function! MSXimBASICRenumLine()

  " Store into 's:renumLine' the first line number to be used by the final
  " MSX-BASIC program.  The command '#renumLine' can be used to set the
...
function! MSXimBASICProcedureCall()

  " Store into 's:procedureCall' the command used in the source by the
  " MSX-BASIC program.  The command '#procedureCall' can be used to set
...
function! MSXimBASICDefine()
  ...
  call MSXimBASICSaveStep('defined_tags')

endfunction

function! MSXimBASICDefined(needle)
...
function! MSXimBASICLabels()
  ...
  call MSXimBASICSaveStep('labels_joined_to_following_line')
...
"    let l:label=tolower(getreg('l',1))
  ...
  call MSXimBASICSaveStep('label_definitions_removed')
  ...
  call MSXimBASICSaveStep('labels_translated')

endfunction

function! MSXimBASICRenum()
...
"  silent! %substitute/^\s*//e
  ...
  call MSXimBASICSaveStep('line_numbers')
...
function! MSXimBASICSpecialChars()

  " Convert Special characters
  " from the UTF-8 source to the MSX-BASIC charset
  ...
  " N.B.: the following substitutions must be done in the order of the
  " destination character code, to make it impossible that a substitution
  " overwrites a previous one.

  silent! %s/¡/\=nr2char(173)/g
  silent! %s/ª/\=nr2char(166)/g
  silent! %s/«/\=nr2char(174)/g
  silent! %s/º/\=nr2char(167)/g
  silent! %s/»/\=nr2char(175)/g
  silent! %s/¿/\=nr2char(168)/g
  silent! %s/À/\=nr2char(143)/g
  silent! %s/Ä/\=nr2char(142)/g
  silent! %s/Ç/\=nr2char(128)/g
  silent! %s/É/\=nr2char(144)/g
  silent! %s/Ñ/\=nr2char(165)/g
  silent! %s/Ü/\=nr2char(154)/g
  silent! %s/á/\=nr2char(160)/g
  silent! %s/ä/\=nr2char(132)/g
  silent! %s/ç/\=nr2char(135)/g
  silent! %s/è/\=nr2char(138)/g
  silent! %s/é/\=nr2char(130)/g
  silent! %s/ë/\=nr2char(137)/g
  silent! %s/ì/\=nr2char(141)/g
  silent! %s/í/\=nr2char(161)/g
  silent! %s/î/\=nr2char(140)/g
  silent! %s/ï/\=nr2char(139)/g
  silent! %s/ñ/\=nr2char(164)/g
  silent! %s/ò/\=nr2char(149)/g
  silent! %s/ó/\=nr2char(162)/g
  silent! %s/ô/\=nr2char(147)/g
  silent! %s/ö/\=nr2char(148)/g
  silent! %s/ù/\=nr2char(151)/g
  silent! %s/ú/\=nr2char(163)/g
  silent! %s/ÿ/\=nr2char(152)/g
  ...
  call MSXimBASICSaveStep('special_chars_converted')

  let &ignorecase=l:ignoreCaseBackup

endfunction

function! MSXimBASICByte2Char(byte)
...
function! MSXimBASICAddress2Chars(number)
  ...
  echo 'MSXimBASICAddress2Chars(' a:number ')'
  if a:number>65535
    " XXX TODO better
    ...
    return MSXimBASICByte2Char(l:lowByte).MSXimBASICByte2Char(l:highByte)
  endif

endfunction

function! MSXimBASICAddresses2Chars()

  " Convert 16-bit embedded values into two embedded bytes
  " with the format of the selected TAP maker.
  "
  " MSX-imBASIC uses double curly brackets for 16-bit
  ...
  %substitute@{{\([0-9]\{-}\)}}@\=MSXimBASICAddress2Chars(submatch(1))@ge
  %substitute@{{0x\([0-9A-Fa-f]\{-}\)}}@\=MSXimBASICAddress2Chars(str2nr(submatch(1),16))@ge

endfunction

function! MSXimBASICChars()

  " Convert all special chars

  call MSXimBASICAddresses2Chars()
  call MSXimBASICSpecialChars()
...
function! MSXimBASICBasFile()

  " Create a copy of the current MSX-imBASIC file
...
"  silent cd %:h
  ...
  silent update " Write the current MSX-imBASIC file if needed
...
function! MSXimBASICDSKFile()

  " Create a DSK image disk with the BAS file.

  let s:dskFilename=expand('%:r').'.dsk'
  echo "\n"
  silent! execute '!wrdsk '.s:dskFilename.' '.s:basFilename
...
function! MSXimBASICSaveStep(description)
...
function! MSXimbasSaveVariables()
...
function! MSXimbasRestoreVariables()
  " Restore the variables that were changed
  let &ignorecase=s:ignoreCaseBackup
  let &shortmess=s:shortmessBackup
endfunction

function! MSXimbas2bas()

  " Convert the content of the current Vim buffer, a MSX-imBASIC source, to
  " its MSX-BASIC equivalent.
  "
  " This is an entry function of the converter. This function can be called
  " using a Vim key mapping (as defined in <~/.vim/ftplugins/msximbasic.vim>),
  " manually executed with ':call MSXimbas2bas()' or called from the provided
  " command line wrapper <msximbas2bas.sh>.


  call MSXimbasSaveVariables()
  ...
  let s:stepsDir=s:sourceFileDir.'/msximbas2dsk_steps/'
  ...
  echo "\n"
  echo 'msximbas2dsk (version '.s:version.') by Marcos Cruz (programandala.net)'
  echo "\n"

  " Conversion steps
  call MSXimBASICBasFile()
  call MSXimBASICInclude()
  call MSXimBASICConfig()
  call MSXimBASICConditionalConversion()
  call MSXimBASICClean()
  call MSXimBASICVim()
  call MSXimBASICControlStructures()
  call MSXimBASICLabels()
  call MSXimBASICRenum()
  call MSXimBASICChars()
  ...
  call MSXimbasRestoreVariables()

endfunction

function! MSXimbas2dsk()

  " Convert the content of the current Vim buffer, a MSX-imBASIC source, to a
  " DSK file with its MSX-BASIC equivalent.
  "
  " This is an entry function of the converter. This function can be called
  " using a Vim key mapping (as defined in <~/.vim/ftplugins/msximbasic.vim>),
  " manually executed with ':call MSXimbas2dsk()' or called from the provided
  " command line wrapper <msximbas2dsk.sh>.

  call MSXimbas2bas()
  call MSXimBASICDSKFile()
